...
const VocabularyModule = (() => {
  ...
  function renderMatching(panel, words) {
    ...
    wordsTitle.style.cssText = 'font-family:var(--font-ui);color:#555;margin-bottom:10px;text-align:center;';
    wordsTitle.textContent = 'Words';
    wordsCol.appendChild(wordsTitle);

    const defsCol = document.createElement('div');
    const defsTitle = document.createElement('h4');
    defsTitle.style.cssText = 'font-family:var(--font-ui);color:#555;margin-bottom:10px;text-align:center;';
    defsTitle.textContent = 'Definitions';
    defsCol.appendChild(defsTitle);
    ...
    // Create word elements with direct onclick
    shuffledW.forEach(v => {
      ...
      el.onclick = function() {
        ...
        if (existing) {
          ...
          existing.dEl.classList.remove('paired', 'selected');
          pairs = pairs.filter(p => p !== existing);
          selectedWordEl = null;
          return;
        }
        // Deselect other words
        wordsCol.querySelectorAll('.match-word').forEach(w => {
          if (!w.classList.contains('locked') && !w.classList.contains('paired')) {
            w.classList.remove('selected');
          }
        });
        el.classList.add('selected');
        selectedWordEl = el;
        console.log('selectedWordEl set to:', selectedWordEl.dataset.word);
      };
      wordsCol.appendChild(el);
    });

    // Create definition elements with direct onclick
    shuffledD.forEach(v => {
      const el = document.createElement('div');
      ...
      el.dataset.word = v.word;
      el.onclick = function() {
        console.log('Definition clicked:', v.definition, 'selectedWordEl:', selectedWordEl ? selectedWordEl.dataset.word : 'null');
        if (el.classList.contains('locked') || !selectedWordEl) return;
        const existing = pairs.find(p => p.dEl === el);
        if (existing) {
          existing.line.remove();
          existing.wEl.classList.remove('paired', 'selected');
          el.classList.remove('paired', 'selected');
          pairs = pairs.filter(p => p !== existing);
          return;
        }
        ...
        pairs.push({ wEl: selectedWordEl, dEl: el, word: selectedWordEl.dataset.word, line: line, locked: false });
        selectedWordEl = null;
      };
    ...
    hint.style.cssText = 'font-family:var(--font-ui);font-size:0.85em;color:#666;margin-top:10px;text-align:center;';
    hint.textContent = '① Click a word ② Click its definition → blue line ③ Click paired item to unpair ④ Submit when all paired';
    panel.appendChild(hint);

    submitBtn.onclick = function() {
      if (pairs.length < words.length) {
        fb.className = 'explanation-box incorrect';
        fb.textContent = 'Pair all ' + words.length + ' words first! (' + pairs.length + '/' + words.length + ')';
        return;
      }
      let correct = 0;
      pairs.forEach(p => {
        if (p.word === p.dEl.dataset.word) {
          p.locked = true;
          p.wEl.classList.remove('paired');
          p.wEl.classList.add('locked', 'correct-match');
          p.dEl.classList.remove('paired');
          p.dEl.classList.add('locked', 'correct-match');
          p.line.remove();
          p.line = drawLine(p.wEl, p.dEl, '#4caf50', false);
          correct++;
          StarSystem.earn('vocab-match-' + p.word);
        } else {
          p.line.remove();
          p.wEl.classList.remove('paired');
          p.wEl.classList.add('wrong-match');
          p.dEl.classList.remove('paired');
          p.dEl.classList.add('wrong-match');
          setTimeout(() => {
            p.wEl.classList.remove('wrong-match');
            p.dEl.classList.remove('wrong-match');
          }, 1500);
        }
      });
      ...
      // Re-render word wall
      const vwPanel = panel.closest('.activity-layout').querySelector('#vwPanel');
      if (vwPanel) renderWordWall(vwPanel, words);
      if (correct === words.length) {
        fb.className = 'explanation-box correct';
        fb.textContent = '🎉 All ' + words.length + ' matched!';
        submitBtn.disabled = true;
        submitBtn.textContent = 'All Done!';
      } else {
        fb.className = 'explanation-box incorrect';
        fb.textContent = correct + '/' + words.length + ' correct. Retry the rest!';
      }
    };
        ...
        el.onclick = function() {
          ...
          if (w === item.word) {
            ...
            setTimeout(() => {
              ...
              blank.classList.remove('filled', 'wrong-fill');
            }, 1000);
          }
        };
        bank.appendChild(el);
